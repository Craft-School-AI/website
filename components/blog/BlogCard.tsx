...
export function BlogCard({
  ...
  post: BlogPost;
  /** Свежая статья: на широких экранах занимает 2×2 ячейки сетки */
  featured?: boolean;
}) {
  // Наклон висит на обёртке, а не на самой ссылке: BlogCard остаётся
  // серверным компонентом (post содержит иконку-функцию), а hover-сдвиг
  // и рост тени на Link продолжают работать поверх 3D-наклона
  return (
    <TiltCard className={featured ? 'lg:h-full' : ''}>
      <Link
        href={`/blog/${post.slug}`}
      ...
      >
        ...
        <div className={`relative z-10 p-5 ${featured ? 'lg:p-8' : ''}`}>
          ...
          <h2
            className={`heading-md mt-2 text-balance text-ivory ${
          ...
          <p
            className={`mt-2 line-clamp-2 text-sm leading-snug text-ivory/75 ${
              featured ? 'lg:line-clamp-3 lg:max-w-xl lg:text-base' : ''
            }`}
          >
            {post.description}
          </p>
